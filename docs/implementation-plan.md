...
| 19.5   | Approval Web Dashboard                                                                                                                                                                                                                                                    | ✅ Done (2026-07-20) |
...
| 35     | Update approval card: per-component, advisory breaking-changes section                                                                                                                                                                                           | ✅ Done (2026-07-27) |
...
| 40     | approval notification cards + dismissal service call; --mode notifications                                                                                                                                                                                                | ✅ Done (2026-07-27) |
| 41     | Notifications tab in dashboard: pending, history, filters                                                                                                                                                                                                        | ✅ Done (2026-07-27) |
...
| 56     | Card-type dispatch: utils/card_types.py; card_type field on all approval cards; dispatch table in approve()                                                                                                                                                               | ✅ Done (2026-07-29) |
...
| 71     | add_tool registration: migration v9 (registered_tools), ToolExecutor._dynamic_tools, CARD_TYPE_CODE_PROPOSAL, dashboard approval handler, user_tools/ loader on startup                                                                                                   | ✅ Done (2026-07-31) |
...
| 76     | `cloud_spend` DB migration v15; `BillingCapError`; `CARD_TYPE_CLOUD_ESCALATION` approval card; re-run with `ClaudeAPIClient` on approval                                                                                                                                  | ✅ Done (2026-08-07) |
...
| 84     | Autonomous gap detection: finish_repair with capability_gap=True triggers propose_patch → sandbox_code → code_proposal approval card automatically                                                                                                                        | ✅ Done (2026-08-11) |
...
### Phase 4.5 — Approval UX ✅ Complete (2026-07-20)
...
### Phase 7 — Evidence Capture and Approval Display ✅ Complete (2026-07-21)
...
Items 83–86. The sandbox infrastructure (`read_source`, `propose_patch`, `sandbox_code`, code_proposal approval card) was delivered in Phase 17.5. This phase adds the autonomous trigger (agent detects a capability gap during a repair loop), the `open_pr` path (formal PR instead of in-process registration), a security review, and ADR 007. Requires Milestones 7 and 9 complete.

| Items | Concern |
| ----- | ------- |
| 83    | open_pr tool: gh pr create; builds on propose_patch + sandbox_code from item 70; PR body template |
| 84    | Autonomous gap detection: finish_repair with capability_gap=True triggers propose_patch → sandbox_code → code_proposal approval card |
...
### Phase 25 — NetAlertX Platform Management ✅ Complete (2026-08-12)
...
| B | Separate-machine Docker installer + MQTT routing | ✅ Done (2026-08-12) — PR #212 |
| C | Disk space checks + setup.sh improvements + supervisor migration offer | ✅ Done (2026-08-12) — PR #214 |
| D | Full MQTT topic coverage + event deduplication | ✅ Done (2026-08-12) — PR #216 |
