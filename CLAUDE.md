...
**All AgentLoop submissions must go through `PueoWorkQueue`** — use `get_work_queue_or_none()` + `WorkItem`, fall back to direct `await` (or `asyncio.create_task`) only in test/standalone mode when the queue is not initialized. Use `AgentLoop` for any LLM interaction that makes a judgment call, touches HA state, or could benefit from iterative evidence gathering. One-shot LLM calls are permitted only for: (1) volume-throttled streaming pre-filters (`analyze_log_line_with_ai`) where the output is purely a binary gate before a queue submission, and (2) secondary enrichment calls made INSIDE an already-running tool executor (`_enrich_fix_context`). Judgment-call classifiers are not a valid use case for one-shot. See ADR 018, ADR 025.
