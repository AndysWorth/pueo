...
class TestChatDebugLog:
    """Placeholder: /chat/sessions/{id}/debug-log removed in debug overhaul (#574).

    The endpoint was replaced by HTML episode reports served from
    /debug-episodes/session_{id}/index.html. Tests for the new endpoint live in
    tests/test_debug_mode.py::TestDebugModeAPI.
    """

    def test_old_debug_log_endpoint_gone(self, db_path, monkeypatch):
        import web.dashboard as dashboard
        from fastapi.testclient import TestClient

        monkeypatch.setattr(dashboard, "DB_PATH", db_path)
        client = TestClient(dashboard.app, raise_server_exceptions=False)
        assert client.get("/chat/sessions/1/debug-log").status_code == 404
