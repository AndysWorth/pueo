...
from config import HA_HOST, HA_USER, SSH_KEY_PATH, LOG_REMOTE_PATH, OLLAMA_MODEL, CONFIDENCE_THRESHOLD
...
class LogEvaluation(BaseModel):
    is_actionable: bool = Field(description="True if this log indicates a configuration or integration issue that a code patch can fix.")
    root_cause_summary: str = Field(description="Brief string summarizing exactly what failed (e.g., 'Malformed YAML in light integration').")
    confidence_score: float = Field(description="Value between 0.0 and 1.0 evaluating certainty.")
...
async def analyze_log_line_with_ai(log_line: str) -> LogEvaluation:
    ...
    try:
        response = await asyncio.to_thread(
            ...
            options={"temperature": 0.0}, format=LogEvaluation.model_json_schema()
        )
        ...
        return LogEvaluation(is_actionable=False, root_cause_summary="Inference crash", confidence_score=0.0)
    ...
    try:
        async with asyncssh.connect(HA_HOST, username=HA_USER, client_keys=[SSH_KEY_PATH], known_hosts=None) as conn:
            async with conn.create_process(tail_command) as process:
                print("👁️  System Listening. Monitoring for smart home infrastructure anomalies...")
                ...
                async for line in process.stdout:
                    ...
                    if CRITICAL_LOG_PATTERN.search(clean_line):
                        ...
                        print(f"🧠 Consulting local {OLLAMA_MODEL} for triage classification...")
                        evaluation = await analyze_log_line_with_ai(clean_line)

                        print(f"📋 AI Evaluation -> Actionable: {evaluation.is_actionable} | Cause: {evaluation.root_cause_summary}")

                        if evaluation.is_actionable and evaluation.confidence_score > CONFIDENCE_THRESHOLD:
...
async def trigger_remediation_pipeline():
    ...
    try:
        ...
        print(f"❌ [Remediation Route Failed] Could not execute swap pipeline: {e}", file=sys.stderr)
